...
jQuery.extend(Payment.prototype, {
	...
	save: function() {
		checkout.collectErrorsFlag = 1;
		var validator = jQuery(this.form).validate(validatorSettings);
		if (checkout.loadWaiting != false)
			return;
		this._beforeSave();
		if (validator.form()) {
			checkout.setLoadWaiting('payment');
			jQuery(this.form).append('<div id="temporary_password_div" style="display:none"><input type="hidden" name="password" value="'+jQuery('#login-password').val()+'"</div>');
			jQuery.post(jQuery(this.form).attr('action'), jQuery(this.form).serialize(), this.nextStep, 'json');
...
jQuery.extend(Review.prototype, {
	...
	save: function() {
		...
		{
			var params = jQuery(payment.form).serialize();
		}
