	...
	if(navigator.userAgent.indexOf('IE 8')!=-1){
         function replacingClass () {
		   ...
		   if (jQuery(document.body).width() > 479 && jQuery(document.body).width() < 768) { //iPhone
			...
			jQuery('.grid-2 .products-grid.widget-grid > li:nth-child(4n+4)').removeClass('omega').next().removeClass('alpha clear-2')
			jQuery('.grid-2 .products-grid.widget-grid > li:nth-child(6n+6)').addClass('omega').next().addClass('alpha clear-2')
			...
			if (jQuery(document.body).width() > 767 && jQuery(document.body).width() < 977){ //Tablet
				...
				jQuery('.more-views > ul > li').removeClass('clear-2');
				jQuery('.more-views > ul > li:nth-child(2n+2)').next().addClass('clear-2');
				jQuery('#footer .newsletter-line .grid_12').css('backgroundSize', '50%');
			}  
			...
			if(jQuery('.widget-latest').length){
				var gridMargin = parseFloat(jQuery('.widget-latest li.grid_3').css('margin-left'));
				jQuery('.widget-latest li.item').css({
					'margin-left' : gridMargin,
					'margin-right' : gridMargin
				});
				jQuery('.widget-latest li.grid_3:nth-child(4n+1)').css({
					'clear': 'left',
					'margin-left': 0
				});
				jQuery('.widget-latest li.grid_3:nth-child(4n+4)').css({
					'clear': 'right',
					'margin-right': 0
				});
			}
