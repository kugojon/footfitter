...
jQuery(window).load(function() {
 ...
 function wideMenuPos() {
  ...
  /* Coin Slider */
	...
	/* Product tabs */
 	if(jQuery('.product-tabs-widget').length){
		function productTabs(){
			jQuery('ul.product-tabs li:not(.current)').on('click', function() {
				jQuery('.product-tabs-widget .product-tabs-box').removeClass('visible')
				index = jQuery(this).index();
				jQuery(this).addClass('current').siblings().removeClass('current');
				jQuery('div.product-tabs-widget .product-tabs-box').eq(index).addClass('visible');
				labelsHeight();
			});
		}
	...
	function checkoutSteps(mode){
		if(jQuery('#checkoutSteps').length){
			if(mode == 'start'){
				jQuery('#checkoutSteps li.section .buttons-set button.button').on('click.checkstep', function(){
					holder = jQuery(this).parents('li.section').attr('id');
					
					function search(){
						activeHolder = jQuery('#checkoutSteps li.section.active').attr('id');
						
						if(holder != activeHolder){
							position = Math.floor(jQuery('#'+activeHolder).offset().top);
				...
				jQuery('#checkoutSteps li.section .buttons-set button.button').off('click.checkstep');
			}
		}
	}
	...
	if (jQuery('#categories-accordion').length){
		jQuery('#categories-accordion li.parent ul').before('<div class="btn-cat closed"><i class="fa fa-plus-square-o"></i><i class="fa fa-minus-square-o"></i></div>');
		jQuery('#categories-accordion li.parent.active > .btn-cat').removeClass('closed').parent().addClass('touched opened');
		...
		jQuery('#categories-accordion li.parent .btn-cat').each(function(){
			jQuery(this).on('click', function(){
				if(!jQuery(this).parent().hasClass('touched') && menuAction == true){
					jQuery(this).parent().addClass('touched opened').children('ul').slideDown(200);
					jQuery(this).removeClass('closed');
					document.addEventListener('touchstart', categoryMenuListener, false);
				}else if(menuAction == true){
					jQuery(this).parent().removeClass('touched opened').children('ul').slideUp(200);
					jQuery(this).addClass('closed');
					document.removeEventListener('touchstart', categoryMenuListener, false);
				}
			});
		});
		function categoryMenuListener(e){
			var touch = e.touches[0];
			if(jQuery(touch.target).parents('.block-vertical-nav').length == 0 && !jQuery(touch.target).hasClass('block-vertical-nav')){
				menuAction = false;
				jQuery('#categories-accordion li.parent.touched .btn-cat').addClass('closed');
				jQuery('#categories-accordion li.parent.touched').removeClass('touched opened').children('ul').slideUp(200);
				document.removeEventListener('touchstart', categoryMenuListener, false);
				menuActionReset();
			}
		}
