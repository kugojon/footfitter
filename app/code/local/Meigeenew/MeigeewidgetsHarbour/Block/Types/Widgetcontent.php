    ...
    protected function _optionToHtml($element, $option, $selected)
    {
		...
		<script type='text/javascript'>
			function hideLabels(labelsAttr){
				labelsAttr = ['price', 'add_to_cart', 'wishlist', 'compare', 'rating_stars', 'rating_cust_link', 'rating_add_review_link', 'product_name', 'timer'];
			...
			itemsHandler(['.product_name', '.rating_stars', '.rating_cust_link', '.rating_add_review_link', '.price',  '.add_to_cart', '.wishlist', '.compare', '.timer-box']);
			...
			function optionsReader(){
				parameters = ['price', 'add_to_cart', 'wishlist', 'compare', 'rating_stars', 'rating_cust_link', 'rating_add_review_link', 'product_name', 'timer-box'];
				setTimeout(function(){
					$('widget_options').childElements().each(function(section){
						if(!section.hasClassName('no-display')){
			...
			function templateActions(){
				function templateHandler(event){
					...
					if((event.target.value.indexOf('footer_list.phtml') !=-1)){
						event.target.up(2).select('.widget-holder')[0].select('.rating_stars, .reviews, .rating_cust_link, .rating_add_review_link, .review-divide, .timer-box').each(function(item){
							item.hide();
						});
					} else {
						event.target.up(2).select('.widget-holder')[0].select('.rating_stars, .reviews, .rating_cust_link, .rating_add_review_link, .review-divide, .timer-box').each(function(item){
